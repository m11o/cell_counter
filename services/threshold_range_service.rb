...
class ThresholdRangeService
  ...
  def call!
    add_component_with_constraints(0, 0, 1, 1) { JLabel.new 'Threshold' }
    add_component_with_constraints(0, 1, 1, 1) { JLabel.new '最小値' }
    add_component_with_constraints(1, 1, 1, 1) { ConfigTextField.new :threshold_min, '', MAX_TEXT_FIELD_COUNT }
    add_component_with_constraints(0, 2, 1, 1) { JLabel.new '最大値' }
    add_component_with_constraints(1, 2, 1, 1) { ConfigTextField.new :threshold_max, '', MAX_TEXT_FIELD_COUNT }
